...
export const projects: Project[] = [
  ...
  {
    id: "ai-quote-multi-agent",
    title: "AI 报价多 Agent 协作系统",
    oneLiner:
      "把客户询价文本拆成需求分析、商品匹配、定价与风险审核，输出可供销售核对的报价草案。",
    tags: ["AI应用", "多Agent", "B端售前", "产品验证"],
    background:
      "B2B 售前询价常散落在聊天与邮件中，商品匹配慢、报价口径不统一，风险往往事后才发现。本项目以本地 Demo 验证：多个专职 Agent 如何协作处理询价，同时保证金额只来自定价引擎。当前 LLM 为规则 Mock，用于验证流程与边界，而非已上线 SaaS。",
    problem:
      "销售需要把自然语言询价变成可审核方案；若字段提取靠猜、金额由模型改写、内部成本与对外报价混在一起，既难审计，也容易出错或泄露。",
    solution:
      "设计线性工作流：询价 → 需求分析 → 商品匹配 → Python 定价 → 风险审核 → 报价方案。定价只能走定价引擎，Agent 不得改数；风险在出方案前打标；前端区分内部工作台与客户预览，对外隐藏成本价。缺关键字段时暂停澄清，而不是硬生成。",
    role: "负责售前询价流程拆解与 Agent 职责边界设计，明确定价单一来源、风险前置和内外视图分离，并推动 Demo 主路径与评估方式验证。",
    features: [
      "需求解析：将询价文本结构化，缺失字段进入澄清暂停",
      "商品匹配：Top-K 检索与候选选择，避免一次暴露全库",
      "定价引擎：金额只来自 Python 计算，模型只做理解与文案",
      "双视图：内部审核看风险与协作轨迹，客户预览只展示可对外信息",
    ],
    aiWorkflow: [
      {
        title: "写清 Agent 做什么、不做什么",
        detail:
          "把理解、匹配、文案交给 Agent，把算价和风控留给规则引擎，避免模型直接改价。",
      },
      {
        title: "流程与失败分支",
        detail:
          "补齐字段缺失、无法匹配、待人工审核等分支，保证 Demo 能演示停下来的情况。",
      },
      {
        title: "Cursor 落地工作台",
        detail:
          "使用 Cursor 辅助实现 FastAPI 编排与 React 双视图，打通询价文本到报价草案。",
      },
      {
        title: "评估验证",
        detail:
          "用离线用例检查流程是否跑通、定价是否与引擎一致、客户预览是否泄露内部信息。",
      },
    ],
    gallery: [
      {
    ...
    summary:
      "该项目验证了：AI 报价产品的关键不是让模型直接给价，而是把流程、计算和风险边界拆开。Demo 用于证明主路径与失败分支可跑通，评估只说明规则 Mock 下的流程正确性，不外推为真实大模型或上线效果。",
  },
]
